function z = MyRbfpreimg_opt(varargin)
% MYRBFPREIMG_OPT RBF pre-image problem by Gradient optimization. Adapted
% from rbfpreimg.m of the STPR Toolbox (Franc and Hlavac).
%
% Synopsis:
%  z = MyRbfpreimg_opt(model)
%  z = MyRbfpreimg_opt(model,options)
%
%  Description:
%   z = MyRbfpreimg_opt(model) it uses gradient method to solve 
...
%     direction.
%
%   z = MyRbfpreimg_opt(model,options) use to specify the control
...
%--------------------
fval = myGaussianKernelMatrix(Z,model.sv.X,arg)*model.Alpha(:);
fval = -fval.^2;
...
while change > options.min_improvement,
   ...
   % compute gradient   
   %dotp = kernel( model.sv.X,z,ker,arg ).*model.Alpha(:);  % STPR Toolbox.
   dotp = myGaussianKernelMatrix( model.sv.X,z,arg ).*model.Alpha(:);
   ...
   [t,fval] = fminunc('MyRbfpreimg_opt',options.start_t,opt,model.Alpha,Ai,Bi,C);
